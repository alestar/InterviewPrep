	...
	for i in range(1, amount + 1):
		for curr_coin in range(len(coins)):
			...
			if coins[curr_coin] <= i:
				...
				# If the remaining amount count is the min, then add one (to total amount of coin counts) for this round pick,
				# because there is still a coin to be selected/added to complete the current amount
				arr_counts[i] = min(curr_amount_count, remaining_amount_count + 1)
