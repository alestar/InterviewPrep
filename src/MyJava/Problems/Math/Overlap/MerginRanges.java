package MyJava.Problems.Math.Overlap; /**
...
public class MerginRanges {
    ...
    public List<Meeting> mergeRanges(List<Meeting> meetings) {
        ...
        // sort by start time
        Collections.sort(sortedMeetings, new Comparator<Meeting>() {// nlogn
            @Override
        ...
        for (Meeting currentMeeting : sortedMeetings) {// n

            Meeting lastMergedMeeting = mergedMeetings.get(mergedMeetings.size() - 1);

            // if the current meeting overlaps with the getLast merged meeting, use the
